...
import { Flex, Box, Image, Heading,Text,Spacer, useColorModeValue,Button } from '@chakra-ui/react';
...
function FichaAlumno() {
  ...
  return (
    ...
    <Button position="absolute" right="10" color="white" size="sm" onClick={handleBackClick} ><ArrowLeftIcon  mr="10px"/> Volver </Button>
      <Box 
      ...
      >
        ...
        <Text size="sm" alingText="center" pl="8px" fontWeight="Bold">{`${alumno.apellido}, ${alumno.nombre}`}</Text>
        <Text color='gray' mt="10px">Número DNI:</Text>
        <Text size="sm" pl="8px" fontWeight="semibold">{alumno.dni}</Text>
        <Text color='gray' mt="10px">Legajo:</Text>
        <Text size="sm" pl="8px" fontWeight="semibold">{alumno.legajo}</Text>
        <Text color='gray' mt="10px">Email:</Text>
        <Text size="sm" pl="8px" fontWeight="semibold">{alumno.email}</Text>
        <Text color='gray' mt="10px">Celular</Text>
        <Text size="sm" pl="8px" fontWeight="semibold" mb="20px">{alumno.telefono}</Text>
        
        <hr></hr>
        
        <Text color='gray' mt="20px">Compromiso de Pago: </Text>
        <Text size="sm" alingText="center" pl="8px" fontWeight="Bold">Firmado</Text>
        <Text color='gray' mt="10px">Estado:</Text>
        <Text size="sm" pl="8px" fontWeight="semibold" mb="20px">{alumno.estado}</Text>
