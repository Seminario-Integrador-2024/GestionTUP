...
import { Container, Box, Text, Heading, List, ListItem, VStack } from '@chakra-ui/react';
...
const ListadoMaterias: React.FC = () => {
    ...
    const handleMateriaClick = (materia: string) => {
        const materiaLink = LINK_MATERIAS.find((item: MateriaLink) => item.title === materia);

        console.log('Materia:', materia);
        console.log('Materia Link:', materiaLink);

        if (materiaLink) {
            console.log('Navigating to:', `/estadisticas/alumnos-que-cursan-materia/${materiaLink.url}`);

            navigate(`/estadisticas/alumnos-que-cursan-materia/${materiaLink.url}`);
        } else {
    ...
    return (
        <Container maxW="container.md" p={4}>
            <VStack spacing={6} align="start">
                <Heading as="h1" size="lg">Listado de Materias</Heading>
                <Box w="full">
                    <CustomSelect
                    ...
                    <Box w="full">
                        <List spacing={3}>
                            {filteredSubjects.map(materia => (
                                ...
                                >
                                    <Text fontSize="md" color="gray.700">{materia}</Text>
                                </ListItem>
                            ))}
                        </List>
                    </Box>
