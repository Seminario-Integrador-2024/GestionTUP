...
export default function TablaMaterias() {
  ...
  const handleAgregar = (
    ...
    try {
      ...
      const Data = FetchPostMateria(
        parseInt(codigo_materia),
        parseInt(anio_cursada),
        parseInt(anio_plan),
        nombre,
    ...
    setMaterias([
      ...materias,
      { codigo_materia, anio_cursada, anio_plan, nombre, cuatrimestre },
    ]);
    onClose2();
  };
  ...
  const handleConfirmarEditar = (
    ...
    try {
      const Data = FetchPutMateria(
        parseInt(codigo_materia),
        parseInt(anio_cursado),
        parseInt(anio_plan),
        nombre,
        parseInt(cuatrimestre)
      );
      ...
      const index = materias.findIndex(
        (materia) => materia.codigo_materia === parseInt(codigo_materia)
      );
      if (index !== -1) {
    ...
    <Box>
      <Box>
        <Flex justifyContent="center" mb={4}>
          <Button leftIcon={<AddIcon />} colorScheme="green" onClick={onOpen2}>
            Agregar Materia
          </Button>
        </Flex>
